...
async def on_message(message):
    if message.author == bot.user:
        return
    if 'loot' in message.content.lower() or 'goblin' in message.content.lower():
        ...
        voice.play(discord.FFmpegPCMAudio("sounds/goblin_laugh.mp3"))
...
async def on_message(message):
    if message.author == bot.user:
        return
    if 'oh boy' in message.content.lower():
        await message.channel.send('*Netski Voice*: DID SOMEBODY SAY RAID?!?!?!?!?!')
    #if message.author.name == 'Heist':
    #    await message.channel.send('Disclaimer: Village Idiot speaking.')
...
async def gg(ctx, users: Greedy[User], *, value):
    # Send a message to the mentioned user!
    for user in users:
        print(users)
        user_send = await bot.get_user_info(user)
